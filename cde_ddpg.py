...
class Replay:
    ...
    def sample_batch(self, batch_size=32):
        return random.sample(self.memo, batch_size)
...
class DDPG:
    ...
    def __init__(self, act_dim, env_dim, act_range, buffer_size = 20000, gamma = 0.99, lr = 0.00005, tau = 0.001):
        ...
        self.buffer = Replay()
####################################################################################################################
        self.curiosity = Curiosity(self.env_dim, self.act_dim)
####################################################################################################################
        self.batch_size = 64
    ...
    def update_models(self, states, actions, critic_target, next_state):
...
####################################################################################################################
        ...
        self.curiosity.train_on_batch(states, actions,next_state)
####################################################################################################################
    ...
    def train(self, env):
        ...
        tqdm_e = tqdm(range(100000), desc='Score', leave=True, unit=" episodes")
        success = []
        test_reward = []
        ...
        for e in tqdm_e:
            ...
            while not done:
                ...
                new_state, r_ext, done, _ = env.step(a)
                new_state = get_state(env)

####################################################################################################################
                
                
                r = r_ext + self.curiosity.get_curious_reward(old_state,a,new_state,self.iteration)
                self.iteration += 1
####################################################################################################################
                ...
                for t in range(step_size):
                    ...
                    if t == step_size - 1:
                        reward = 0.1
                    experience.append((old_state, action, reward, done, next_state, gripperPos, gripperOrn, next_gripperPos, next_gripperOrn, blockPos, blockOrn))
                ...
                if r > 0.9 and done:
                    ...
                    test_reward.append(self.test(env))
    ...
    def train_batch(self):
        if len(self.buffer.memo) > self.batch_size:
            ...
            # for state, action, reward, done, new_state in sample_batch:
                ...
                self.update_models(state, action, critic_target,new_state)
    ...
    def test(self,env):
        ...
        print("The average reward is {}".format(total_reward/50))
...
if __name__ == '__main__':
    ...
    ddpg = DDPG(n_action, n_state, env.action_space.high,lr = 0.0005)
